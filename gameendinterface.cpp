...
GameEndInterface::GameEndInterface(QWidget *parent):
...
void GameEndInterface::keyPressEvent(QKeyEvent * event){

    if (event->key()==Qt::Key_Escape){
            this->close();
            this->MusicOff();}
}
void GameEndInterface::Musicon(){
    player=new QMediaPlayer();
   player->setVolume(100);
   player->setMedia(QUrl("qrc:/music/Music/background_music/RISE.mp3"));
    player->play();
}
void GameEndInterface::MusicOff(){
    player->stop();
}
